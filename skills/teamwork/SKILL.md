...
- A member whose model keeps aborting a legitimate turn — a refusal, a safety classifier, a real-time safeguard — is **model-blocked, not dead** (PROTOCOL §Liveness): the lead reconstructs it from `.team/` and respawns the role on a different model. Carry the exact error the pane shows to the human — they decide whether it is a real limit or a false positive to route around.
