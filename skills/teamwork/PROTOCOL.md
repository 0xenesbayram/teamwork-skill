...
A task is **done** when it meets the mission's verification bar — the standard the lead recorded in Decisions — with the evidence noted on the board. The bar is domain-shaped: green tests and a clean build for code; source-cited coverage for research and analysis; a probed-everything checklist for security. Whatever it is, it must be checkable — an owner can tell done from not-done — not "looks finished". A checkable bar still hides one thing: it can be met against a *subset* of the work and still read as done — green tests on the module you touched but not its sibling with the same bug, cited coverage from the source-type you consulted but not the one that contradicts it, a checklist clearing the handler you found but not the second one behind the same input. So a done-note names *what it was met against* — which modules, sources, or paths — not just that the bar was met; a note that shows effort but not its scope, on work that plausibly has more than one path, is a done the owner re-opens. Two things the first mission had to learn by amending its bar mid-flight, so set them at the start: measure against a **production build**, since dev servers emit framework noise that makes a "zero console errors" clause unmeetable in dev and trivial in prod; and **batch evidence re-capture at the end of a round** rather than per change, because a bar that re-shoots everything on every tweak makes each round cost more than the fix. And a task that produced a reusable instrument is not done until that instrument is in the vault (§Vault) — promoted now, not deferred to the retrospective.

One of those vault checks is mechanical, so run it as a gate, not an intention — the lead re-runs it at the integration check (§Roles): **every vault entry carries its `verified` line — `grep -rL 'verified:' .team/vault --include='*.md'` comes back empty before a task goes `done`.** The line was mandatory already (§Vault); the omission is what recurs, so catch it with `grep`, not a reminder. This checks the line is *present*, not that it is *true* — a stale or lying `verified:` still passes, and only re-running an instrument against something it should fail on catches that (§Vault). Then wake whoever the board shows is waiting on it.
...
- **Model-blocked** — alive, but the member's model keeps aborting a legitimate turn: a safety classifier firing, a refusal, a real-time safeguard tripping on the content the task hands it. The turn dies before it can produce, and feeding it more of the same input only re-trips it, so it is not coming back on its own. It is not Killed — its context and claims are intact, and that context is expensive — so recovery runs a **two-rung ladder, cheapest first.** Rung one: **switch the model in place with `/model`** — the block is model-specific (the model that trips is not the one to re-run the seam on), and an in-place switch keeps the context, so try it before you throw it away. Rung two, only if the switch can't land: **the lead reconstructs the member's state from what already reached `.team/`** — the board, the commits, any handoff or evidence it wrote (which is why members check work in as they go, §Commits, not in one closing turn) — and **respawns the role on a different model.** Either way, tell the anchor the exact error the pane shows, so the human can see whether it is a real limit or a false positive to route around.
- **Handoff** — the only planned one.

**Treat roster `active` as a renewable lease, not a sticky flag.** A member is `active` only while its pane keeps heart-beating — its `Ctx:` reading moves; a reading that has stopped is a lapsed lease, and the sweep exists to catch it. A quota-pause also stops the reading but resumes with its context, so a stale pane is never a kill until the lead has read it (the pane shows which of the modes above it is). The lead sweeps the roster **on a cadence, not on a hunch** — at every milestone and at least every twenty minutes. `tmux ls` plus one `capture-pane` per member costs nothing, interrupts nobody, and reads context and idleness in the same pass (§Handoff). The board stays loud while one member is stuck, and the busiest boards hide the most. A member marked active that hasn't moved and doesn't answer is one of the first four — find out which before recording it as progress; the pane shows which, and a model abort appears in it verbatim. If instead the pane shows a line still sitting unsubmitted in the prompt box, the member is mechanically stuck — a swallowed Enter, not one of the stop modes — and the fix is to resubmit the prompt (§Communication), not respawn. On a first mission two members sat dead at a usage limit for eight hours while the roster still called them active and the board still showed their tasks claimed.
...
**The lead hands off like anyone else — and it is the one member that can't be trusted to call its own handoff.** The lead crosses the band too, and a session watching its own context is exactly the thing that fails when its context degrades. So detection is external: the roster sweep reads the lead's row alongside the members' (§Liveness), and when the lead is past the band the **anchor** orders and sequences the handoff — the successor is spawned before the lead tears down. The ordering stays outside the lead because the lead is the coordinator whose judgment the band is degrading.
...
4. Lead writes `.team/RETRO.md` in three passes, because the retro→amendment pipeline is the construct's own worst habit — left alone it only ever *adds prose*, and a coordination protocol that only grows stops being read:
   - **What happened, and what it changed** — the run narrative, where the war-story numbers live.
   - **A compaction pass — retire / compress / relocate — required, the same standing as the §Vault de-dup that already runs at retro.** Which existing rules did this mission prove redundant, superseded, or never-enforced-so-gate-or-cut? This is a *pass*, not a count-quota: do not cut a load-bearing rule to make room for a new one — the question is only what this run proved to be dead weight. Decoupling it from the add-count is the point; a run that surfaces a real new failure class *should* add.
   - **Proposed amendments, each gated at the point it is proposed** — every rule the mission had to learn mid-flight becomes a proposal carrying three fields the lead fills (the forcing function is on the author):
     - `Home: PROTOCOL / SKILL / vault — why here, and not one level out?` An amendment that cannot justify the *coordination protocol* as its home does not enter it. Most good fixes are a discipline the protocol already runs somewhere applied to the place that lacks it, or a codebase/tooling concern that belongs in the vault or the narrative SKILL — pushing one out to where it belongs is a healthy subtraction, not a demotion.
     - `Supersedes: <the exact rule or § this replaces — or "none: new class">`. If it replaces standing text, the edit that lands it **rotates the superseded text to `.team/PROTOCOL-HISTORY.md` in the same change** — kept and dated, never deleted, exactly as a settled Decision rotates to `DECISIONS.md` (§TEAM.md). Compaction that *silently drops* standing policy is its own failure mode; the cold archive is how it stays honest.
     - **Number check — parameter or measurement?** A number that is part of the rule (the context band, the member cap, the 300-character Decision limit, the sweep cadence) is a *parameter* and stays in the text. A number that is one mission's reading (a context a session happened to reach, a file size this run hit, a date) is a *measurement* — it belongs in the narrative or `PROTOCOL-HISTORY.md`, never the rule, because it dates and swells the doc without binding anything. War-story numbers teach well in the SKILL and the narrative, not in the binding protocol.

   The anchor puts the surviving proposals to the human — and **rejects an unjustified `PROTOCOL` home before it reaches the human**: a proposal whose `Home` field doesn't earn the coordination protocol goes back to land in the SKILL, a specialist skill, or the vault instead. The protocol changes only through the human.
5. Lead commits `.team/vault/`, `.team/retro/`, `.team/RETRO.md`, and any `.team/PROTOCOL-HISTORY.md` rotation, and reports retro-done to the anchor.
